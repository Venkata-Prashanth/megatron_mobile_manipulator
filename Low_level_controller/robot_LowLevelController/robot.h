...
public:
  ...
  Motor(uint8_t encoderCh, uint8_t phaseAPin, uint8_t phaseBPin, uint8_t pinPus,
      ...
      : encoder(encoderCh, phaseAPin, phaseBPin, pinPus),
        encoderRes(encoderResolution){};
};

class Robot {
private:
  static const float wheelRadius, trackWidth, maxRPM,
      maxVel; // robot kinematics variables
...
public:
  ...
  void moveRobot(geometry_msgs__msg__Twist *cmdvel_msg,
