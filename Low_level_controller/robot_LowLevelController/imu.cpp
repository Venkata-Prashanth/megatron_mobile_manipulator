...
void IMU::imuInit(uint8_t csPin, SPIClass &spiPort, uint32_t spiFreq) {
  ...
  // The next few configuration functions accept a bit-mask of sensors for which
...
void IMU::updateData(sensor_msgs__msg__Imu *imu_msg) {
  ...
  imu_msg->angular_velocity.x =
      (double)(calibration->gyro_scaling_factor *
               (myICM.agmt.gyr.axes.x - calibration->gyro_offset[0]));
  imu_msg->angular_velocity.y =
      (double)(calibration->gyro_scaling_factor *
               (myICM.agmt.gyr.axes.y - calibration->gyro_offset[1]));
  imu_msg->angular_velocity.z =
      (double)(calibration->gyro_scaling_factor *
               (myICM.agmt.gyr.axes.z - calibration->gyro_offset[2]));
  ...
  for (i = 0; i < 3; i++)
    temp[i] = (accVal[i] - calibration->accel_bias[i]);
  imu_msg->linear_acceleration.x =
      (double)(calibration->accel_scaling_factor *
               (calibration->accel_a_matrix_inverse[0][0] * temp[0] +
                calibration->accel_a_matrix_inverse[0][1] * temp[1] +
                calibration->accel_a_matrix_inverse[0][2] * temp[2]));
  imu_msg->linear_acceleration.y =
      (double)(calibration->accel_scaling_factor *
               (calibration->accel_a_matrix_inverse[1][0] * temp[0] +
                calibration->accel_a_matrix_inverse[1][1] * temp[1] +
                calibration->accel_a_matrix_inverse[1][2] * temp[2]));
  imu_msg->linear_acceleration.z =
      (double)(calibration->accel_scaling_factor *
               (calibration->accel_a_matrix_inverse[2][0] * temp[0] +
                calibration->accel_a_matrix_inverse[2][1] * temp[1] +
  ...
  imu_msg->angular_velocity_covariance[0] =
      calibration->angular_vel_covariance[0];
  imu_msg->angular_velocity_covariance[4] =
      calibration->angular_vel_covariance[1];
  imu_msg->angular_velocity_covariance[8] =
      calibration->angular_vel_covariance[2];

  imu_msg->linear_acceleration_covariance[0] =
      calibration->linear_accel_covariance[0];
  imu_msg->linear_acceleration_covariance[4] =
      calibration->linear_accel_covariance[1];
  imu_msg->linear_acceleration_covariance[8] =
      calibration->linear_accel_covariance[2];
