...
void Robot::moveRobot(geometry_msgs__msg__Twist *step_cmd_msg, unsigned long prev_cmd_time,
                      nav_msgs__msg__Odometry *odom_msg, geometry_msgs__msg__Twist  *motor_msg) {
    ...
    if ((millis() - prev_cmd_time) >= 100) {
        ...
        motor2Right.velReq = step_cmd_msg->linear.x + step_cmd_msg->angular.z;
        motor1Left.velReq =  step_cmd_msg->linear.x - step_cmd_msg->angular.z;
    }

    getRobotVelocity();

    int controlValueLeft = (int)motor1Left.velReq;
    int controlValueRight = (int)motor2Right.velReq;

    
    // For check pid after tuning 
    ...
    if ((millis() - prev_cmd_time) >= 100) {
        ...
        motor2Right.velReq = (step_cmd_msg->linear.x + (step_cmd_msg->angular.z * (trackWidth / 2)));
        motor1Left.velReq = (step_cmd_msg->linear.x - (step_cmd_msg->angular.z * (trackWidth / 2)));
    }
    ...
    */
    ...
    motor_msg->linear.x = controlValueLeft;
    motor_msg->linear.y = motor1Left.velReq;
    motor_msg->linear.z = motor1Left.velActual;
    motor_msg->angular.x = controlValueRight;
    motor_msg->angular.y = motor2Right.velReq;
    motor_msg->angular.z = motor2Right.velActual;
