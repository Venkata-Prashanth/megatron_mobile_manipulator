...
rcl_publisher_t odom_publisher, imu_publisher, motor_publisher;
rcl_subscription_t step_cmd_subscriber;
bool micro_ros_init_successful;

// Ros msg variables
geometry_msgs__msg__Twist step_cmd_msg, motor_msg;
...
void timer_callback(rcl_timer_t *timer, int64_t last_call_time) {
    ...
    if (timer != NULL) {
        ...
        robot.moveRobot(&step_cmd_msg, prev_cmd_time, &odom_msg, &motor_msg);
...
void publishTopics() {
    ...
    rcl_publish(&motor_publisher, &motor_msg, NULL);
...
bool create_entities() {
    ...
    RCCHECK(rclc_subscription_init_default(&step_cmd_subscriber, &node,
                                           ROSIDL_GET_MSG_TYPE_SUPPORT(geometry_msgs, msg, Twist),
                                           CMD_VEL_TOPIC));

    RCCHECK(rclc_publisher_init_default(
        &odom_publisher, &node, ROSIDL_GET_MSG_TYPE_SUPPORT(nav_msgs, msg, Odometry), ODOM_TOPIC));

    RCCHECK(rclc_publisher_init_default(
        &imu_publisher, &node, ROSIDL_GET_MSG_TYPE_SUPPORT(sensor_msgs, msg, Imu), IMU_TOPIC));
    
    RCCHECK(rclc_publisher_init_default(
        &motor_publisher, &node, ROSIDL_GET_MSG_TYPE_SUPPORT(geometry_msgs, msg, Twist), "/motor"));
    ...
    RCCHECK(rclc_executor_add_subscription(&executor, &step_cmd_subscriber, &step_cmd_msg,
                                           &cmdvel_sub_callback, ON_NEW_DATA));
...
void destroy_entities() {
    ...
    rcl_subscription_fini(&step_cmd_subscriber, &node);
