...
const float Robot::maxVel =
    (2 * 3.141592653 * Robot::wheelRadius * Robot::maxRPM) / 60.0;

constexpr ImuCalibration IMU_CALIB = {
    GYRO_SCALING_FACTOR,    GYRO_OFFSET,
    ACCEL_SCALING_FACTOR,   ACCEL_BIAS,
    ACCEL_A_MATRIX_INVERSE, LINEAR_ACCEL_COVARIANCE,
    ANGULAR_VEL_COVARIANCE, ORIENTATION_COVARIANCE};

Robot::Robot(nav_msgs__msg__Odometry *odom_msg, sensor_msgs__msg__Imu *imu_msg)
    : roboclaw(&MOTOR_CONTROLLER_SERIAL_PORT, 10000), imuICM(&IMU_CALIB),
      motor1Left(MOTOR1_ENCODER_CHANNEL, MOTOR1_ENCODER_PIN_A,
                 MOTOR1_ENCODER_PIN_B, 0, ENCODER_RESOLUTION),
      motor2Right(MOTOR2_ENCODER_CHANNEL, MOTOR2_ENCODER_PIN_A,
                  MOTOR2_ENCODER_PIN_B, 0, ENCODER_RESOLUTION) {
  ...
  odom_msg->child_frame_id =
      micro_ros_string_utilities_set(odom_msg->child_frame_id, BASE_FRAME);
...
void Robot::getRobotVelocity() {
  ...
  motor1Left.velActual =
      (((float)leftEncCounts / motor1Left.encoderRes) * wheelRadius * PI * 2) /
      velUpdateTime;
  motor2Right.velActual = (((float)rightEncCounts / motor2Right.encoderRes) *
                           wheelRadius * PI * 2) /
                          velUpdateTime;
}
...
void Robot::updateOdometryData(nav_msgs__msg__Odometry *odom_msg) {
  ...
  odom_msg->twist.twist.linear.x =
      (double)((motor2Right.velActual + motor1Left.velActual) / 2);
...
void Robot::moveRobot(geometry_msgs__msg__Twist *cmdvel_msg,
  ...
  if ((millis() - prev_cmd_time) >= 100) {
    ...
    motor2Right.velReq =
        (cmdvel_msg->linear.x + (cmdvel_msg->angular.z * (trackWidth / 2)));
    motor1Left.velReq =
        (cmdvel_msg->linear.x - (cmdvel_msg->angular.z * (trackWidth / 2)));
  }
  ...
  int controlValueLeft = motor1Left.pid.update(
      motor1Left.velReq, motor1Left.velActual, velUpdateTime, coastflag);
  int controlValueRight = motor2Right.pid.update(
      motor2Right.velReq, motor2Right.velActual, velUpdateTime, coastflag);
